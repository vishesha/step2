...
router.post("/insurancesubmit", (req, res) => {
  setupDatabase()
    .then((db) => {
      try {
        ...
        const checkQuery =
          "SELECT * FROM insurance_selection_form WHERE teamName = ?";
        db.query(checkQuery, [teamName], (checkError, checkResult) => {
          ...
          if (checkResult.length > 0) {
            ...
            const updateQuery = `
              UPDATE insurance_selection_form
              SET coverage = ?, level = ?
              WHERE teamName = ?
            `;
            db.query(
            ...
            const insertQuery = `
              INSERT INTO insurance_selection_form (teamName, coverage, level)
              VALUES (?, ?, ?)
            `;
            db.query(
              insertQuery,
...
router.post("/getInsuranceSelectionForms", (req, res) => {
  setupDatabase()
    .then((db) => {
...
router.post("/getAllInsuranceSelectionForms", (req, res) => {
  setupDatabase()
    .then((db) => {
      // Extract the username from the request body

      // Define the SQL query to fetch insurance selection data for the specified user
      const fetchQuery = "SELECT * FROM insurance_selection_form";
